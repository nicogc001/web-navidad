(() => {
    ...
    async function loadProductos() {
      ...
      try {
        ...
        if (!r.ok) throw new Error(productos?.error || "Error cargando productos");
        ...
        grid.innerHTML = productos
          .map((p) => {
            ...
            <div class="product-card">
              <div class="card-media" style="${img ? `background-image:url('${img}')` : ""}; background-size:cover; background-position:center;"></div>
              <div class="card-body">
                ...
                <div class="small muted" style="margin-top:6px">
                  Stock:
                  <input type="number" min="0" step="1" value="${Number(p.stock ?? 0)}"
                    class="form-control form-control-sm d-inline-block" style="width:110px"
                    data-stock="${p.id}" />
                  <button class="btn btn-sm" type="button" data-save-stock="${p.id}">Guardar</button>
                </div>
    ...
    function estadoBadge(estado) {
      if (estado === "pendiente") return `<span class="badge text-bg-warning">Pendiente</span>`;
      if (estado === "confirmado") return `<span class="badge text-bg-primary">Confirmado</span>`;
      if (estado === "entregado") return `<span class="badge text-bg-success">Entregado</span>`;
    ...
    async function loadPedidos() {
      ...
      try {
          ...
          .map((p) => {
              ...
              .map(
                (it) =>
                  `<li class="small text-muted">
                    ${escapeHtml(it.producto_nombre)} x${Number(it.cantidad)} — ${money(Number(it.precio_unitario) * Number(it.cantidad))}
                  </li>`
              )
              .join("");
            ...
            <div class="card card-glass">
              <div class="card-body">
                <div class="d-flex justify-content-between align-items-start gap-3">
                  <div>
                    ...
                    <div class="small text-muted">
                      ${escapeHtml(p.nombre_cliente)} · ${escapeHtml(p.telefono)}
                    </div>
                    <div class="small text-muted">
                    📅 ${p.fecha_recogida ? fmtDate(p.fecha_recogida) : "Sin fecha"} ·
                    📍 ${p.lugar_recogida ? escapeHtml(p.lugar_recogida) : "Sin lugar"} ·
                ...
                ${p.observaciones ? `<div class="mt-2 small text-muted">📝 ${escapeHtml(p.observaciones)}</div>` : ""}
                ...
                <div class="d-flex flex-wrap gap-2 mt-3">
                  <button class="btn btn-sm" type="button" data-estado="confirmado" data-pid="${p.id}">
                    Confirmar
                  </button>
                  <button class="btn btn-sm" type="button" data-estado="entregado" data-pid="${p.id}">
                    Entregado
                  </button>
                </div>
              </div>
        ...
        pedidosGrid.querySelectorAll("[data-estado]").forEach((btn) => {
          btn.addEventListener("click", () => {
            const pid = btn.getAttribute("data-pid");
            const estado = btn.getAttribute("data-estado");
            patchEstadoPedido(pid, estado);
          });
        });
      } catch (e) {
